...
plt.xlabel("Weekdays")
plt.title("Steps taken during the week")
...
fig.legend(["Student 1", "Student 2"])# Your code here)
...
plt.bar(regions, revenue[:, 0])
plt.bar(regions, revenue[:, 1], bottom=revenue[:, 0])
plt.bar(regions, revenue[:, 2], bottom=revenue[:, 0]+revenue[:, 1])
