...
# **Your university mail**: ktulva@uos.de
# 
# ___
# 
...
plt.plot (weekdays, alice_steps, color= "red", linestyle= "--", linewidth= 3)
plt.plot(weekdays, bob_steps, color= "blue", linestyle= "-.", linewidth= 3)
plt.plot(weekdays, carol_steps, color= "green", linestyle= ":", linewidth= 3)
...
plt.xlabel("Days")
plt.ylabel("Steps")
plt.title("Steps Taken by Students in Weekdays")
...
Mean1 = 70
Std1 = 20
student1_scores = np.random.normal(Mean1,Std1,100) 

Mean2 = 80
Std2 = 10
student2_scores = np.random.normal(Mean2,Std2,100)  # Set Mean

len(student1_scores)

# Set the bin sizes for the histograms
bin_size_student1 =  round((np.max(student1_scores) - np.min(student1_scores) ) /len(student1_scores))
bin_size_student2 =  round((np.max(student2_scores) - np.min(student2_scores) ) /len(student2_scores))

# Create a figure with two subplots
fig, axes = plt.subplots(1,2,figsize=(10,4))

# Plot histogram for Student 1
axes[0].hist(student1_scores, bins=range(50,101,bin_size_student1),color='blue', label='Student 1')
axes[0].set_title("Student 1 Math Exam Score Distribution")
axes[0].set_xlabel("Scores")
axes[0].set_ylabel("Frequency")

# Plot histogram for Student 2
axes[1].hist(student2_scores, bins=range(50,101,bin_size_student1),color='green', label='Student 2')
...
for i in range(revenue.shape[0]):
    plt.bar(regions, revenue[i], bottom = np.sum(revenue[:i], axis = 0), label= categories[i], color= colors[i])
