...
temperature = [ random.uniform(20, 30), random.uniform(20, 30), random.uniform(20, 30), random.uniform(20, 30), random.uniform(20, 30), random.uniform(20, 30), random.uniform(20, 30)]
...
# Scatter plot
plt.scatter(weekdays, alice_steps, marker ='o' , color='red',  label='Alice', zorder= 1)
plt.scatter(weekdays, bob_steps, marker ='s', color='blue',   label ='Bob', zorder = 1)
plt.scatter(weekdays, carol_steps, marker ='^', color='green', label   ='Carol', zorder = 1)

# Line connecting scatterplot points
plt.plot ( weekdays, alice_steps, zorder = 2  , color = 'red', linestyle = '--')
plt.plot ( weekdays, bob_steps, zorder = 2 , color = 'blue', linestyle = '--')
plt.plot ( weekdays, carol_steps, zorder = 2, color = 'green' , linestyle = '--')
# Legend
...
student1_scores = np.random.normal(loc = 70, scale = 10, size = 100) # Mean: 70, Standard Deviation: 10
student2_scores = np.random.normal(loc = 80 , scale = 8, size = 100)   # Mean: 80, Standard Deviation: 8
...
axes[0].set(title = "Student 1 Math Exam Score Distribution")
axes[0].set( ylabel = "Scores")
axes[0].set(xlabel = "Frequency")

# Plot histogram for Student 2
axes[1].hist(student2_scores, bins=range(50, 101, bin_size_student2), color='red', alpha=0.7, label = 'Student 2')
axes[1].set(title = "Student 2 Math Exam Score Distribution")
axes[1].set( ylabel = "Scores")
...
revenue = np.array([[100, 200, 150],
                    ...
                    [80, 100, 200],
                    [180, 120, 90]])
revenue_new = np.array([[100, 150, 80, 180], [200, 120, 100, 120], [150, 80, 200, 90]])

plt.bar( regions, height = revenue_new[0], bottom = 0, color = 'red', label = 'Electronics')
plt.bar(regions, height = revenue_new[1], bottom = revenue_new[0], color = 'green', label = 'Clothing')
plt.bar(regions, height = revenue_new[2], bottom = revenue_new[1]+revenue_new[0], color = 'blue', label = 'Books')
