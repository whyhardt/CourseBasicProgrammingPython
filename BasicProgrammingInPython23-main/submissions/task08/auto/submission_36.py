...
temperature = np.random.randint(20, 31, size=7)

plt.plot(days, temperature, color='purple')
plt.title('Weekly Temperature')
plt.xlabel('Days of the Week')
plt.ylabel('Themperature (°C)')
...
plt.plot(weekdays, alice_steps)
plt.plot(weekdays, bob_steps)
plt.plot(weekdays, carol_steps)
...
student1_scores = np.random.normal(70, 10, 100) #, 100)  # Mean: 70, Standard Deviation: 10
student2_scores = np.random.normal(80, 8, 100) #, 100)   # Mean: 80, Standard Deviation: 8
...
print('Electronics: ',revenue[:, 0]) #I just made this to see/check that I was choosing the correct ones 
print('Clothing: ', revenue[:, 1])
print('Books: ', revenue[:, 2])

plt.bar(regions, revenue[:, 0], color='red', label=categories[0])
plt.bar(regions, revenue[:, 1], bottom=revenue[:, 0], color='green', label=categories[1])
plt.bar(regions, revenue[:, 2], bottom=revenue[:, 0]+revenue[:, 1], color='blue', label=categories[2])
