...
temperature = np.random.randint(20, 31, size=len(days), dtype=int)
...
# Add your code here
plt.plot(weekdays, alice_steps, color='red', linestyle='solid', linewidth=1)
plt.plot(weekdays, bob_steps, color='blue', linestyle='dotted', linewidth=1)
plt.plot(weekdays, carol_steps, color='green', linestyle='dashed', linewidth=1)
...
plt.bar(regions, revenue[:,1], color='green', label=categories[1], bottom=revenue[:,0])
plt.bar(regions, revenue[:,2], color='blue', label=categories[2], bottom=revenue[:,0]+revenue[:,1])
