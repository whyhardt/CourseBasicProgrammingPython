...
# **Your name here**: Johannes Voth  
# **Your university mail**: jvoth@uni-osnabrueck.de
# 
# ___
# 
...
temperature = np.random.randint(20, 30, size=7)
...
plt.plot(weekdays, alice_steps, color='red', linestyle='dashed', linewidth=2)
...
# Set the colors for each product category
colors = ['red', 'green', 'blue']

# Plot the stacked bar chart
plt.bar(regions, revenue[:, 0], color=colors[0], label=categories[0])
for i in range(1, len(categories)):
    plt.bar(regions, revenue[:, i], bottom=np.sum(revenue[:, :i], axis=1), color=colors[i], label=categories[i])
