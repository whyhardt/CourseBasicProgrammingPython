...
min_tmp , max_tmp = 20, 30
temperature = np.random.randint(min_tmp, max_tmp + 1, size=7)

plt.plot(days, temperature, color='purple', marker='o', linestyle='--', label='Temperature (°C)')
plt.title("Weekly Temperature")
plt.xlabel("Days of the Week")
plt.ylabel("Temperature (°C)")

plt.ylim(min_tmp - 1, max_tmp + 1)
plt.grid(True)
plt.legend()
...
# Scatter plot
plt.scatter(weekdays, alice_steps, marker='o', color='red', label='Alice', zorder=2)
plt.scatter(weekdays, bob_steps, marker='s', color='blue', label='Bob', zorder=2)
plt.scatter(weekdays, carol_steps, marker='^', color='green', label='Carol', zorder=2)

# Line connecting scatterplot points
plt.plot(weekdays, alice_steps, color='red',linewidth=0.7 , zorder=1)
plt.plot(weekdays, bob_steps, color='blue',linewidth=0.7 , zorder=1)
plt.plot(weekdays, carol_steps, color='green',linewidth=0.7 , zorder=1)
...
student1_scores = np.random.normal(70,10 ,size=100)  # Mean: 70, Standard Deviation: 10
student2_scores = np.random.normal(80,8 ,size=100)   # Mean: 80, Standard Deviation: 8
...
axes[0].hist(student1_scores, bins=range(50, 101, bin_size_student1), color='blue', alpha=0.7 , label='Student 1')
axes[0].set_title("Student 1 Math Exam Score Distribution")
axes[0].set_xlabel("Scores")
axes[0].set_ylabel("Frequency")

# Plot histogram for Student 2
axes[1].hist(student2_scores, bins=range(50, 101, bin_size_student2), color='green', alpha=0.7 , label='Student 2')
...
index = 0
accumelated_revenue = np.sum(revenue[:,:index], axis=1)
plt.bar(regions, revenue[:, index], bottom=accumelated_revenue, label=categories[index], color='red')

index = 1
accumelated_revenue = np.sum(revenue[:,:index], axis=1)
plt.bar(regions, revenue[:, index], bottom=accumelated_revenue, label=categories[index], color='blue')

index = 2
accumelated_revenue = np.sum(revenue[:,:index], axis=1)
plt.bar(regions, revenue[:, index], bottom=accumelated_revenue, label=categories[index], color='green')
