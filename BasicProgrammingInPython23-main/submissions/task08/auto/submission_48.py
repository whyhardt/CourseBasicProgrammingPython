...
temperature = np.random.uniform(20, 30, size=len(days))

plt.plot(temperature, days, color= "purple")
plt.title("temperatures of the week")
...
# Line connecting scatterplot points #so much pain ;-;
plt.plot(weekdays, alice_steps, color="red", linestyle="-",linewidth=5) 
plt.plot(weekdays, bob_steps, color="blue", linestyle="-",linewidth=5)
plt.plot(weekdays, carol_steps, color="green", linestyle="-", linewidth=5)
#linewidth=5 looks nice
# Legend
plt.legend()
...
import matplotlib.pyplot as plt
import numpy as np
#forgot this part and lead to very much pain

# Generate sample scores for two students
student1_scores = np.random.normal(100, 10, 70) # Your code here, 100)  # Mean: 70, Standard Deviation: 10
student2_scores = np.random.normal(100, 8, 80) # Your code here, 100)   # Mean: 80, Standard Deviation: 8

# Set the bin sizes for the histograms
bin_size_student1 = 1 # Your code here
bin_size_student2 = 2 # Your code here
...
colors= ["red", "blue", "green"] #added colors
...
plt.bar(regions,revenue[:,0], color=colors[0],label=categories[0])
plt.bar(regions,revenue[:,1], bottom=revenue[:,0], color=colors[1], label=categories[1])
plt.bar(regions,revenue[:,2], bottom=revenue[:,0] +revenue[:,1], color=colors[2], label=categories[2])
