...
bin_size_student1 = bin_size_1=5
bin_size_student2 = bin_size_2=5
...
axes[1].set.xlabel("Scores")
axes[1].set_ylabel("Frequency")

# Add a legend to differentiate the two students
fig.legend(["Student1", "Student 2"])
...
for i, cat in enumerate (categories):
    bottom = np.sum(revenue[:, :i], axis=1)
    ax.bar (regions, revenue[:,i], bottom=bottom, label=cat, color=colors[i]))
