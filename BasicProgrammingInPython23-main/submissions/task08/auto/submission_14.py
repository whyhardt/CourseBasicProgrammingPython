...
temperatures = np.random.uniform(20, 30, size=7)
...
colors = ['blue', 'orange', 'green']  # Define the colors for the bars

plt.bar(regions, revenue[:, 0], color=colors[0])
plt.bar(regions, revenue[:, 1], bottom=revenue[:, 0], color=colors[1])
plt.bar(regions, revenue[:, 2], bottom=revenue[:, 0] + revenue[:, 1], color=colors[2])

plt.title('Sales Revenue by Region and Product Category')
plt.xlabel('Regions')
plt.ylabel('Revenue')
plt.legend(categories)  # Add legend with product categories
