...
plt.plot(weekdays, alice_steps, color='red', linestyle='-', label='Alice')
plt.plot(weekdays, bob_steps, color='blue', linestyle='-', label='Bob')
plt.plot(weekdays, carol_steps, color='green', linestyle='-', label='Carol')
...
plt.title("Steps taken through the week by each student")
...
plt.bar(regions, revenue[:, 0], color='red', label=categories[0], bottom=np.sum(revenue[:, :0], axis=1))
plt.bar(regions, revenue[:, 1], color='green', label=categories[1], bottom=np.sum(revenue[:, :1], axis=1))
plt.bar(regions, revenue[:, 2], color='blue', label=categories[2], bottom=np.sum(revenue[:, :2], axis=1))
