...
temperature = [random.uniform(20, 30) for _ in range(len(days))]
...
# Customize the line by setting the color, style, and width to make it clearly visible.
plt.plot(weekdays, alice_steps, linewidth=1, color='red', linestyle='--')
plt.plot(weekdays, bob_steps, linewidth=1, color='blue', linestyle=':')
plt.plot(weekdays, carol_steps, linewidth=1, color='green', linestyle='-')
...
plt.title('Step Analysis for each day of the week')
...
axes[0].legend(["Student 1"], loc='upper right')#, bbox_to_anchor=(1.0, 1.0))
axes[1].legend(["Student 2"], loc='upper right')#, bbox_to_anchor=(1.0, 1.0))
...
plt.bar(regions, revenue[:, 0], label=categories[0], color="r")
plt.bar(regions, revenue[:, 1], bottom=revenue[:, 0], label=categories[1], color="g")
plt.bar(regions, revenue[:, 2], bottom=revenue[:, 0]+revenue[:, 1], label=categories[2], color="b")
