...
# Set up temperature array with random integers
temperature = np.random.randint(20,30,7)

# plotting & adding title and labels
plt.plot(days,temperature, color = "purple")
plt.title("Weekly temperature",color="green")
plt.xlabel("Days of the week", color="blue")
plt.ylabel("Temperature", color="blue")

#adjust y- axis such that the scale is more visible

plt.yticks(np.arange(0,39,10))
...
#for easier access to legend
fig, ax = plt.subplots()

# Scatter plot
plt.scatter(weekdays, alice_steps, marker='o', color='red', label='Alice')
plt.scatter(weekdays, bob_steps, marker='*', color='blue', label='Bob')
plt.scatter(weekdays, carol_steps, marker='D', color='green', label='Carol')

# Line connecting scatterplot points
plt.plot(weekdays,alice_steps,c="r",linestyle = "--")
plt.plot(weekdays,bob_steps,c="b",linestyle = "--")
plt.plot(weekdays,carol_steps,c="g",linestyle = "--")

# Legend
ax.legend()

# Axes labels and title
plt.title("Steps taken in a week",c="C4")
plt.xlabel("Weekdays",c="C4")
plt.ylabel("Number of steps",c="C4")
...
# for easier access in plotting the bars
x = regions
...
#transpose the array, else it won't fit 
revenue = np.transpose(revenue)

# for easier plotting of the stack
y1= revenue[0,0:]
y2= revenue[1,0:]
y3= revenue[2,0:]
