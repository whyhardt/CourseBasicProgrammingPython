...
# **Your university mail**: srebers@oni-osnabrueck.de
# 
# ___
# 
...
temperature = np.random.randint(20,31, size=len(days))
...
plt.plot(weekdays,alice_steps, color='salmon', ls="--", lw="2")
plt.plot(weekdays,bob_steps, color='lightskyblue', ls="--", lw="2")
plt.plot(weekdays,carol_steps, color='lightgreen', ls="--", lw="2")

# Legend
plt.legend(loc='lower right')

# Axes labels and title
plt.title("Steps taken throughout the weekdays", {'size':15})
plt.xlabel("Days")
...
# Create a figure with two subplotsx
...
axes[0].set_title("Student 1 Math Exam Score Distribution", size="10")
...
axes[1].set_title("Student 2 Math Exam Score Distribution", size="10")
...
plt.bar(regions, [revenue[i][0] for i in [0,1,2,3]], color="red", label="Electronics")
plt.bar(regions, [revenue[i][1] for i in [0,1,2,3]], bottom=[revenue[i][0] for i in [0,1,2,3]], color="green", label="Clothing")
plt.bar(regions, [revenue[i][2] for i in [0,1,2,3]], bottom=[revenue[i][0] + revenue[i][1] for i in [0,1,2,3]], color="blue", label="Books")
