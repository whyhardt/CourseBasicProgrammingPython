...
axes[1].set_title("Student 1 Math Exam Score Distribution")
...
plt.bar(np.arange(len(regions)), revenue[:, 0], color='red')
plt.bar(np.arange(len(regions)), revenue[:, 1], bottom=revenue[:, 0], color='green')
plt.bar(np.arange(len(regions)), revenue[:, 2], bottom=np.sum(revenue[:, :2], axis=1), color='blue')
