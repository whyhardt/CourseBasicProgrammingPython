...
temperature = np.random.randint(20, 31, 7)    #generate seven random ints between 20 and 30
plt.plot(days,temperature, color = "purple")   #plot and set color to purple
plt.title("Weekly Temperature")  #title of the plot
plt.xlabel("Days of the Week")   #x-axis name
plt.ylabel("Temperature (°C)")   #y-axis name
...
plt.bar(regions, revenue[:, 0], color = "red", label = "Electronics")
plt.bar(regions, revenue[:, 1], bottom = revenue[:, 0],color = "green", label = "Clothing")
plt.bar(regions, revenue[:, 2], bottom = revenue[:, 0] + revenue[:, 1], color = "blue", label = "Books")
