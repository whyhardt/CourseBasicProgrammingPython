...
# **Your name here**: Joshua Stock
# **Your university mail**: jostock@uos.de
# 
# ___
# 
...
plt.plot(weekdays, alice_steps, linestyle="--", color='red', linewidth="3")
plt.plot(weekdays, bob_steps, linestyle="-.", color='blue', linewidth="3")
...
helpvariable = np.arange(len(revenue))

plt.bar(helpvariable, revenue[:,0], bottom=0, color="red")
plt.bar(helpvariable, revenue[:,1], bottom=revenue[:,0], color="green")
plt.bar(helpvariable, revenue[:,2], bottom=np.sum(revenue[:, :2],axis=1), color="blue")
