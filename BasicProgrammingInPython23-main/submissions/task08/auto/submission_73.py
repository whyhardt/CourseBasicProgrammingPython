...
temperature = np.random.uniform(20, 30, size=7)
...
plt.plot(weekdays, bob_steps, color='green', linestyle='-', linewidth=1)
plt.plot(weekdays, carol_steps, color='blue', linestyle='-', linewidth=1)

# Legend
plt.legend()

# Axes labels and title
plt.title('Number of Steps Taken by Students')
plt.xlabel('Days of the Week')
...
student1_scores = np.random.normal(70, 7, 100)  # Mean: 70, Standard Deviation: 10
student2_scores = np.random.normal(80, 8, 100)   # Mean: 80, Standard Deviation: 8

# Set the bin sizes for the histograms
bin_size_student1 = range(75, 101, 5)
bin_size_student2 = range(75, 101, 5)

# Create a figure with two subplots
fig, axes = plt.subplots(1, 2, figsize=(10, 4))

# Plot histogram for Student 1
axes[0].hist(student1_scores, bins= bin_size_student1, color='blue', alpha=0.7)
axes[0].set_title("Student 1 Math Exam Score Distribution")
axes[0].set_xlabel("Scores")
axes[0].set_ylabel("Frequency")

# Plot histogram for Student 2
axes[1].hist(student2_scores, bins= bin_size_student2, color='green', alpha=0.7)
...
plt.bar(regions, revenue[:, 1], label=categories[1], color='green')
plt.bar(regions, revenue[:, 2], label=categories[2], color='blue')
