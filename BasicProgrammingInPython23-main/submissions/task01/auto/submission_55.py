...
for i in range(num_cities):
    ...
    city_population = int(input("Enter the city's population: "))
...
def add_book(collection, book_id, title, author, year):
    collection.update({book_id : [title, author, year]})
...
class StudentGradeTracker:
    def __init__(self):
        self.student_grade_tracker = dict()

    def add_grade(self, student_id, grade):
        if not student_id in self.student_grade_tracker:
            self.student_grade_tracker[student_id] = grade
        else:
    ...
    def remove_grade(self, student_id):
        if student_id in self.student_grade_tracker:
            self.student_grade_tracker.pop(student_id)
            print("The student's grade has been removed.")
        else:
            print("The student has no grade recorded.")

    def display_grades(self):
        if self.student_grade_tracker:
            for student_id, grade in self.student_grade_tracker.items():
...
grade_tracker.add_grade("003", 89)
grade_tracker.display_grades()
