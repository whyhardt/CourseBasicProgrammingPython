...
# Price is first set to 0, as people who are not allowed to ride don't have to pay anything.
# Leaving it out would lead to an error in the next cell in case of height < 120 (as price would not be defined)
# or, if the code was executed before with height >= 120, the price would still be the same as for the person before.
price = 0  
allowed = False  

if height < 120:
    print("You cannot ride the rollercoaster at the moment.")
else:
    # Everyone with height >= 120 is allowed, only price varies with age:
    allowed = True
    if age < 12:
        price = 5
    elif age <= 18:
