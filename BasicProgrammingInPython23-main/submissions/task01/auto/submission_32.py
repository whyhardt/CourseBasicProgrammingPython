...
if height < 120:
    print("Unfortunately, you are not allowed to ride the rollercoaster at the moment.")
    allowed = False
    
    
else:
    age = int(input("You are allowed to ride the rollercoaster. How old are you?: "))
    allowed = True
    if age < 12:
        print("The price is 5 Euros.")
        price = 5
    elif (age >= 12) & (age <= 18):
        print("The price is 8 Euros.")
        price = 8
    else:
        print("The price is 15 Euros.")
        price = 15
...
print(f'The person (height={height}{",age=" + str(age) if allowed else ""}) is{" not" if not allowed else ""} allowed to take the ride {"and their price is " + str(price) + "€" if allowed else ""} ')
