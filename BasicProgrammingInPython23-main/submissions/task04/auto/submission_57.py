...
def remove_vowels(string):
    ...
    modified_string = ""  #replace vowel with 'nothing'
    for char in string:
        if char not in vowels:  #if the character is not a vowel,
            modified_string += char  #the next character will be studied
    return modified_string   #if the condition is not fulfilled, the vowel will be removed
...
def is_prime(number):
    ...
    for i in range(2, int(number ** 0.5) + 1): #iterate from 2 to the square root of the given number
        if number % i == 0:  #check if the number is divisible evenly by any of these values (=then its not a prime number)
            return False
    return True  #if no factors were found, the number is a prime number
