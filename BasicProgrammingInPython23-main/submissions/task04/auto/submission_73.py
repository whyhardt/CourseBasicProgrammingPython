...
# Default arguments allowing the function to work even if not all four parameter values are given (e.g., if the user wants to sum only 3 values)
...
def intersection(list_1, list_2):
    ...
    for i in list_1:
        # ... check if it is also present in the second list
        if i in list_2:
            new_list.append(i)
    return new_list
...
def remove_vowels(string):
    new_string = ""
    # Check every letter of the string
    for letter in string:
        # If the letter is not a vowel (regardless of the case due to casefold()), add it to the new string
        if letter.casefold() not in ["a", "e", "i", "o", "u"]:
            new_string += letter
    return new_string
...
def is_prime(number):
    ...
    # Check if the number can be divided by a number that is higher than 1 and smaller than itself
    for i in range(2, number):
        # If it can be divided by at least one of these numbers, it is not a prime number
        if number % i == 0:
            return False
        
    # This part of the code is only reached, if the number could not be divided by any number that's neither 1 or itself, so it is a prime number
    return True
...
def fibonacci(n):
    # Base case: the 1st number is 0
    if n == 1:
        return 0
    # Base case: the 2nd number is 1
    elif n == 2:
        return 1
    # Recursive case: sum of the (n-1)th and (n-2)th number
...
def gcd(a, b):
    ...
    # Recursive case: Check for the gcd of b and (a % b)
    return gcd(b, a % b)
