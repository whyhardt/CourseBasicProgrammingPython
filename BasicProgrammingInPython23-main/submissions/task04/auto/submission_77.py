...
def remove_vowels(string):
    Toberemoved = "aeiouAEIOU"
    Output = ""
    for char in string:
        if char not in Toberemoved:
            Output += char
    print(Output)
...
def is_prime(x):
    ...
    if Primestatus == 1:
        ...
        print(x, "is a negative number, therefore it can not be prime number. Atleast according to my quick Google search ^^")
    elif Primestatus == 3:
