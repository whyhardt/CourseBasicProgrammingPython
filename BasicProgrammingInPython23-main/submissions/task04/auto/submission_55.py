...
def intersection(list_a, list_b):
    ...
    for i in range(len(list_a)):
        for j in range(len(list_b)):
            if list_a[i]==list_b[j] and list_a[i] not in list_c: #no double numbers in list_c
...
def remove_vowels(my_str):
    ...
    for vowel, no_vowel in vowel_list:
        if vowel in my_str:
            my_str = my_str.replace(vowel, no_vowel)
    return my_str
...
def is_prime(p):
    if p==0 or p==1:
        return False
    if not (p==0 or p==1):
        for i in range(2,p): #is p divisible by any number from 2 - p-1?
            if p%i==0: #a factor is found, number is dvisible by 1, itself and another factor
                return False
            else: # number p is only divisible by 1 or p
                return True
