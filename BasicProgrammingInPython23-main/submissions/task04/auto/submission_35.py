...
# ### Factorials (2/6 points)
# Create a function ```factorial``` to calculate the factorial of a number (a positive integer) by using recursion.  
# The function should be able to take the number as a parameter and then return its factorial by recursively calling itself in some form.  
# 
...
def gcd(a, b):
    # base case if b yields in zero the gcd is a
    if b == 0:
        return a
    #recursive case
    else:
