...
def is_prime(number):
    ...
    for i in range(2,number):  #check if the number is divisible by a number greater than 1 but smaller than the number itself
        
        if (number % i) == 0:  #check if divisible without a remainder
    
            return False  # if divisible by numbers other then 1 and the number itself
