...
string_a = input("Please write something: ")
string_b = input("Please write something else: ")
...
# The task above says that it should be alphanumeric, but the text that is printed says that it's not.
# I added the "not" to fit to the printed text.
# If the character is supposed to be alphanumerical, the conditional statement would be:
# if char.isalnum():
if not char.isalnum():
...
result = [shopping_list_fruit[2]] + [shopping_list_dairy[0]] + [shopping_list_sweets[0]]
print(result)
...
# Should we only change the three special characters mentioned above or everything that is not a letter, number or point? I did the latter. 

# In[21]:


# Changes everything that is not a letter, number or point

# Checks every character
for char in file_name:
    
    # Replaces the character if it is not a number, letter or point
    # If points, other than the one in front of the file extension, should also be replaced, change the end to:
    # ... or (char == "." and file_name[char:] in file_extensions)
    # with file_extensions being a list with all valid extensions, to check if the dot is followed by an extension.
...
if file_name[-3:] in ["txt", "doc", "pdf"]:
    file_type = "Document file"
    
elif file_name[-3:] in ["jpg", "png"]:
    file_type = "Image file"
    
elif file_name[-3:] in ["mp4", "mov"]:
