...
result = list(filter(lambda x: x == 'Cherry',shopping_list))[0]
print(result)
...
result = shopping_list_dairy + shopping_list_fruit + shopping_list_sweets # the list has at least one element of each type
print(result)
...
unwanted_characters = [":",",","?"]
if any(list(map(lambda x: x in file_name,unwanted_characters))):
...
doc_file_extensions = ["txt","doc","pdf"]
image_file_extensions = ["jpg","png"]
video_file_extensions = ["mp4","mov"]

if any(list(map(lambda x: x in file_name.split('.')[-1],doc_file_extensions))):
    file_type = "Document file"
    
elif any(list(map(lambda x: x in file_name.split('.')[-1],image_file_extensions))):
    file_type = "Image file"
    
elif any(list(map(lambda x: x in file_name.split('.')[-1],video_file_extensions))):
