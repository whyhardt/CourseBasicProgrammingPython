...
result = [shopping_list_fruit[1]] + [shopping_list_dairy[1]] + [shopping_list_sweets[1]]
print(result)
...
for c in file_name:
    if not(c.isalnum()) and c != '.': #since it is **like** colon, coma etc, i chose to see them all as special characters unless dot (cause we need that for the file type)
...
if file_name[len(file_name)-4:len(file_name)] == (".txt" or ".doc" or ".pdf"):
    file_type = "Document file"
    
elif file_name[len(file_name)-4:len(file_name)] == (".jpg" or ".png"):
    file_type = "Image file"
    
elif file_name[len(file_name)-4:len(file_name)] == (".mp4" or ".mov"):
