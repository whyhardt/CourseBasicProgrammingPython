...
# **Your university mail**: adejongh@uni-osnbrueck.de
# 
# ___
# 
...
for i in range(num_cities):
    ...
    pop = int(input("Enter the population if the city: "))
    result = [city, pop]
    cities.append(result)


# Calculate the total population
total_population = sum([el[1] for el in cities])
...
def calculate_average(tuple_students):
    return (tuple_students[1] + tuple_students[2] + tuple_students[3] + tuple_students[4]) / 4
...
def symmetric_difference(set1, set2):
    symm_difference_set = set()
    for elem in set1:
        if elem not in set2:
            symm_difference_set.add(elem)
    for elem in set2:
        if elem not in set1:
            symm_difference_set.add(elem)
    return symm_difference_set
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if grade in self.grades:
            print("The grade of the student with the student id " + student_id + " has been updated")
        self.grades[student_id] = grade

    def remove_grade(self, student_id):
        if student_id in self.grades:
            del self.grades[student_id]
            print("The grade of the student with the student id " + student_id + " has been deleted")
        else:
            print("The student with the student id " + student_id + " does not have a recorded grade")
