...
for i in range(num_cities):
    ...
    city_population = int(input("enter the population number: "))
...
highest_city = max(zip(cities, population), key = lambda x: x [1])

# Find the city with the lowest population
lowest_city = min(zip(cities, population), key = lambda x: x [1])
...
for student_grades in grades:
    ...
    average_grades = calculate_average(*student_grades)
    print(f"{name}: Average Grade = {average_grades}")
...
numbers = # Add implementation here
