...
for i in range(num_cities):
    ...
    cities.append((input("Enter the name of the city: "),int(input("Enter the population of the citiy: "))))
   

# Calculate the total population
total_population = sum(t[1] for t in cities)
...
def calculate_average(x):
  sumgrades = sum(student_grades[1:])
  totalaverage = sumgrades/len(student_grades[1:])
  return totalaverage
...
set1 = set(input("Enter the elements of one set (seperated by spaces): ").split())
set2 = set(input("Enter the elements of another set (seperated by spaces): ").split())
...
def add_book(collection, book_id, title, author, year):
    collection[book_id] = {"title":title, "auther":author, "year":year}
