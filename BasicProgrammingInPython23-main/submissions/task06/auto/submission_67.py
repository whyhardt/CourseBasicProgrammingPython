...
for student_name , *grades in grades:
    name = student_name
...
def symmetric_difference(set1, set2):
  ...
  symmetric_difference_set = union(difference(set1, set2), difference(set2, set1))
...
def count_unique_elements(lst):
  count_of_unique_elements = 0
  for idx , elem in enumerate(lst):
    if elem not in lst[:idx]:
      count_of_unique_elements += 1
      
  return count_of_unique_elements
...
def search_book(collection, book_id):
    if book_id in collection.keys():
        print(f"Book with ID {book_id} found!")
        print(f"Title: {collection[book_id][0]}")
        print(f"Author: {collection[book_id][1]}")
        print(f"Year: {collection[book_id][2]}")
        print("====================================")
    else:
...
def display_all_books(collection):
    for book_id in collection.keys():
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        ...
        if student_id in self.grades.keys():
            print(f"Grade of {grade} for student with ID {student_id} has been updated!")

    def remove_grade(self, student_id):
        if student_id in self.grades.keys():
            del self.grades[student_id]
            print(f"Grade for student with ID {student_id} has been removed!")

        else:
            print(f"Student with ID {student_id} does not have a grade record!")

    def display_grades(self):
        if len(self.grades) == 0:
            print("the grade tracker is empty!, no grades have been recorded yet!")
        
        for student_id in self.grades.keys():
            print(f"Student ID {student_id} : {self.grades[student_id]}")
            print("====================================")
