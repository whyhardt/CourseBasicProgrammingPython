...
# **Your name here**: Paulina Calderon  
# **Your university mail**: jcalderonram@uni-osnabrueck.de
# 
# ___
# 
...
for i in range(#your code here):
    city_name = input ("Enter the name of a city {i+1}:")
    population = int(input("Enter the population of the city {i+1}: "))
    cities.append ((city_name , population))  

# Calculate the total population
total_population = sum(populations for _ , population in cities)
...
def add_book(collection, book_id, title, author, year):
    ...
    collection[boo.id] = book_details
    pass
...
def display_all_books(collection):
    for book_id, book_details in collection.items():
        print ("Book ID:", book_id)
        print ("Title:", book_details["title"])
        print ("Author:", book_details["year"])
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grade_tracker:
            print("Grade for student {student_id} updated")
        pass

    def remove_grade(self, student_id):
        if student_id in self.grade_tracker:
           del self.grade_tracker[student_id]
        print("Grade for student {student_id} has been removed")
        else:
            print("No grade recorded for student {student_id}")
        pass

    def display_grades(self):
        if not self.grade_tracker:
