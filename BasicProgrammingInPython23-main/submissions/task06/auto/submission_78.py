...
for tuple_item in cities:
    city_population.append(tuple_item[1])
...
def calculate_average(tuple_x):
  items_total = sum(tuple_x)
  items_average = items_total/len(tuple_x)
...
set1 = set()
for _ in range(4):
  entry = int(input("Enter a set of numbers: "))
  set1.add(entry)

set2 = set()
for _ in range(4):
  entry = int(input("Enter another set of numbers: "))
  set2.add(entry)

def union(set1, set2):
  two_sets = set1.union(set2)
  return two_sets
...
def count_unique_elements(lst):
  ...
  unique_elements = len(elements_set)
...
def search_book(collection, book_id):
    if book_id in collection:
        print("Book ID:", book_id, "= ", collection[book_id])
...
#I added search_book(book_collection, 2) for the sake of completeness in the output
...
#I changed the location of remove_book() function and added another display_all_books() to see the book with ID 2 in the details of all books in the collection
#Otherwise Book 2 was missing as it was removed
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades:
            self.grades.update({student_id: grade})
    ...
    def remove_grade(self, student_id):
        print("\nGrade update:")
        if student_id in self.grades:
            self.grades.pop(student_id)
            print(f"{student_id}'s grade has been removed")
        else:
            print(f"No grade record for {student_id}")
    ...
    def display_grades(self):
        ...
        if len(self.grades) == 0:
            ...
            for student_id, grade in self.grades.items():
                print("Student ID:", student_id, "Grade =", self.grades[student_id])
