...
num_cities = int(input("What is the number of the cities?: "))

# Iterate over the range of the number of cities
for i in range(num_cities):
    # Get the name and population of the city from the user
    name = input(f"What is the name of the city? {i+1}: ")
    population = int(input(f"What is the population of the city {i+1}: "))

    citylist.append((name, population))

# Calculate the total population
tot_population = sum(population for _, population in citylist)

# Find the city with the highest population
high_city = max(citylist, key=lambda city: city[1])

# Find the city with the lowest population
low_city = min(citylist, key=lambda city: city[1])

# Print the population summary
print("\nPopulation Summary:")
print(f"Sum of the population of all cities: {tot_population}\n")

# Print the city with the highest population
print("City with the highest population:")
print(f"Name: {high_city[0]}")
print(f"Population: {high_city[1]}\n")

# Print the city with the lowest population
print("City with the lowest population:")
print(f"Name: {low_city[0]}")
print(f"Population: {low_city[1]}")
...
def calculate_average(Xtuple):
    average = (Xtuple[1] + Xtuple[2] + Xtuple[3] + Xtuple[4])/4
    return average  
...
for student_grades in grades:
    ...
    ave_grade = calculate_average(student_grades[0:])
    print(f"{name}: Average Grade = {ave_grade}")
...
set1 = set(input("What is your first set?: ").split())
set2 = set(input("What is your second set?: ").split())
...
set_union = sorted(union(set1, set2))
set_intersection = sorted(intersection(set1, set2))
set_difference = sorted(difference(set1, set2))
set_symmetric_difference = sorted(symmetric_difference(set1, set2))
...
uni_count = count_unique_elements(numbers)
print("Number of unique elements:", uni_count)
...
def remove_book(collection, book_id):
    if book_id in collection:
        book_info = collection[book_id]
        print("Title:", book_info['title'])
        print("Author:", book_info['author'])
        print("Year:", book_info['year'])
        
        del collection[book_id]
        print("This book is no longer in your collection.")
    else:
...
def display_all_books(collection):
    if len(collection) == 0:
        ...
        print("collection:")
        for book_id, book_info in collection.items():
...
´show_all_books(book_collection)
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        self.dict[student_id] =  {'Student ID': student_id, 'Grade': grade}

    def remove_grade(self, student_id):
        if student_id in self.dict:
            print("Student ID:", self.dict[student_id]['Student ID'])
            print("Grade:", self.dict[student_id]['Grade'])
        
            del self.dict[student_id]
            print("This student is no longer in your collection.")
        else:
            print("The Student is not in your collection")
    ...
    def display_grades(self):
        if len(self.dict) == 0:
            ...
            for student_id, student_data in self.dict.items():
