...
for i in range(num_cities):
    name = input(f"Enter name of city: ")
    population = int(input(f"Enter population of city: ")
...
def calculate_average(self, math, science, english, history):
    self.math = math
    self.science = science
    self.english = english
    self.history = history
    average = (self.math + self.science + self.english + self.history)/4
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades:
            print("Update grade for student", student_id)
