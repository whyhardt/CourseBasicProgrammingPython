...
for i in range(num_cities):
    city_name = input("Enter Name " + str(i + 1))
    city_population = int(input("Enter Population "+ str(i + 1)))
    raw_population = raw_population + city_population
    cities.append((city_name, city_population))

# Calculate the total population
total_population = raw_population
...
def add_book(collection, book_id, title, author, year):
    collection.update({book_id: {"ID": book_id, "Title": title, "Author": author, "Year": year}})
    return collection
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        self.dictionary.update({student_id: {"ID": student_id,"Grade": grade}})
    ...
    def display_grades(self):
        for x in self.dictionary:
            return print(x, ": ", self.dictionary)
