...
total_population = sum(population for cities, population in cities)
...
def calculate_average(student_grades):
    return (sum(student_grades[1:5])/4)
...
def add_book(collection, book_id, title, author, year):
    book_collection.update({book_id:{"Title: ": title, 
                                     "Author: ": author,
                                     "Year: ": year}})

def remove_book(collection, book_id):
    book_collection.pop(book_id)

def search_book(collection, book_id):
    if book_id in book_collection:
        print(book_collection[book_id])
...
class StudentGradeTracker:
    ...
    def remove_grade(self, student_id):
        if student_id in self.grades:
            ...
            print("This student does not have a recorded grade.")
