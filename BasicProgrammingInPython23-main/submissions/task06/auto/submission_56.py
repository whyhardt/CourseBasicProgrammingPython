...
for i in range(num_cities):
    city_name = input("Enter a name of the city: ")
    city_pop = int(input("Enter population of the city:"))
    i = (city_name, city_pop)
    cities.append(i)

# Calculate the total population
total_population = sum(i[1] for i in cities)

# Find the city with the highest population
highest_city = max(cities, key=lambda i: i[1])

# Find the city with the lowest population
lowest_city = min(cities, key=lambda i: i[1])
...
unique_count = count_unique_elements(new_numbers)    
print("Number of unique elements:", unique_count)

#quick note: I wasn't sure whether unique elements should be defined as elements occuring only once or one unique element 
#corresponds to every different value that is in the list. I followed the first interpretation as it seems to be
...
def add_book(collection, book_id, title, author, year):
    collection[book_id]=title, author, year
...
class StudentGradeTracker:
    ...
    def add_grade(self, st_id, grade):
        self.st_id = st_id
        self.grade = grade
        if not self.grade in self.gt:
            self.gt[self.st_id]=self.grade
        else:
            self.update({self.st_id : self.grade})

    def remove_grade(self, st_id):
        self.st_id = st_id
        del self.gt[self.st_id]
...
grade_tracker.add_grade("001", 69)
