...
for i in range(num_cities):
    helper = [0,0]
    helper[0] = input("Enter city name:")  
    helper[1] = int(input("Enter population:"))
    cities.append(helper) 


# Calculate the total population
total_population = sum(cities[i][1] for i in range(num_cities))

# Find the city with the highest population
highest_city[1] = max(cities[i][1] for i in range(num_cities))
for i in range(num_cities):
    if cities[i][1] == highest_city[1]:
        highest_city[0] = cities[i][0]

# # Find the city with the lowest population
lowest_city[1] = min(cities[i][1] for i in range(num_cities))
for i in range(num_cities):
    if cities[i][1] == lowest_city[1]:
        lowest_city[0] = cities[i][0]
...
def calculate_average(list):
  ...
  average_grade = 0
  for i in iter:
    average_grade = average_grade + list[i]
  average_grade = average_grade/(length-1)
...
def symmetric_difference(set1, set2):
    first = difference(set1,set2)
    second = difference(set2,set1)
...
def count_unique_elements(lst):
  helper = set(lst)
  new_helper = list(helper)
  return len(new_helper)
...
def search_book(collection, book_id):
    if book_id in collection:
        ...
        print("The book could not be found in the collection")
