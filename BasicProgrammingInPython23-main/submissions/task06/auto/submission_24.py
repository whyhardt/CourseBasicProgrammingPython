...
cities = []
cities_populations = []
os = ['osnabrueck', 100000]
cities.append(os)
br = ['bramsche', 30000]
cities.append(br)
ko = ['koeln', 900000]
cities.append(ko)
wa = ['wallenhorst', 40000]
cities.append(wa)
...
for i in range(num_cities):
    this_is_the_number_of_cities_but_why_iterate = i + 1
    cities_populations.append(cities[i][1])

# Calculate the total population
total_population = sum(cities_populations)

# Find the city with the highest population
highest_city = max(cities)

# Find the city with the lowest population
lowest_city = min(cities)
...
def display_all_books(collection):
    for book_id, book_details in collection.items():
        ...
        print("Author:", book_details['author'])
        print("Year:", book_details['year'])
        print()  # Add a new line after each book
...
class StudentGradeTracker:
    ...
    def remove_grade(self, student_id):
        if student_id in self.grades:
            ...
            print(f"Grade removed for Student ID {student_id}.")
        else:
