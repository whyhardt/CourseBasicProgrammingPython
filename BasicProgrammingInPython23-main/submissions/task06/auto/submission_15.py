...
highest_city = sorted(cities, key=lambda x: x[1], reverse=True)[0]


# Find the city with the lowest population
lowest_city = sorted(cities, key=lambda x: x[1])[0]
...
set1 = set(input("Enter numbers of set 1 seperated by spaces:").split())
set2 = set(input("Enter numbers of set 2 seperated by spaces:").split())
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.student_grade:
            print("Grade has been updated!")
        self.student_grade[student_id] = grade

    def remove_grade(self, student_id):
        if student_id in self.student_grade:
            self.student_grade.pop(student_id)
            print("Grade has been removed!")
        else:
            print("Student is not in Grade Tracker!")

    def display_grades(self):
        if len(self.student_grade) > 0:
            for student, grade in self.student_grade.items():
                print(student, " : ", grade)
