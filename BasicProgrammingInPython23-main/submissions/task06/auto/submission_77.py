...
for i in range(num_cities):
    ...
    population = int(input("Enter population of this city: "))
...
def calculate_average(grades):
    average_grade = sum(grades[1:])/len(grades[1:])
...
def remove_book(collection, book_id):
    if book_id in collection:
        ...
        print(f"Book '{book_details['title']}' with ID {book_id} removed successfully.")
    else:
