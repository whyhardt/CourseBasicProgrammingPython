...
# **Your name here**: Seyed Arman Taghizadeh Motlagh  
# **Your university mail**: staghizadehm@uni-osnabrueck.de
# 
# ___
# 
...
for i in range(num_cities):
    #your code here
    cities.append( (input('city name: '), int(input('city population: '))) )

#print(cities)
# Calculate the total population
total_population = sum(n for _, n in cities)
#print(total_population)
# Find the city with the highest population
highest_city = max(cities,key=lambda x:x[1])
    #cities.sort(key=lambda tup: tup[1], reverse=True))
...
def calculate_average(std_grades):
  # Add implementation here
    average = sum(std_grades[1:] ) / 4
    return average
...
def symmetric_difference(set1, set2):
  # Add implementation here
    ...
    for item in set1:
        if item not in set2:
            symm_diff.append(item)
    for item in set2:
        if item not in set1:
            symm_diff.append(item)
    return set(symm_diff)
...
def add_book(collection, book_id, title, author, year):
    # Add implementation here
    return collection.update({str(book_id):(title, author, year)})
    #collection.append((book_id, title, author, year))
    # Remove pass after the implementation
    #pass

def remove_book(collection, book_id):
    # Add implementation here
    return collection.pop(str(book_id), None)

def search_book(collection, book_id):
    # Add implementation here
    if str(book_id) in collection:
        return print(collection[str(book_id)])
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        ...
        self.grade = grade
        grades.update({str(student_id):grade})
    ...
    def display_grades(self):
        ...
        if len(grades) > 0:
            for item in grades:
                print("Student ID: ",item, 'Grade: ' ,grades[item])
        # Remove pass after the implementation
        #pass
grades = {}
