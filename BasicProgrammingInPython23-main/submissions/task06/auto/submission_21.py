...
for i in range(num_cities):
    ...
    population = int(input(f"Population of {name}: "))
...
def remove_book(collection, book_id):
    if book_id in collection:
        ...
        print(f"Book with ID {book_id} is not in collection.\n")

def search_book(collection, book_id):
    if book_id in collection:
        print(f"Book ID: {book_id} -> {collection[book_id]}\n")
    else:
        print(f"Book with ID {book_id} is not in collection.\n")

def display_all_books(collection):
    for key, value in collection.items():
        print(f"Book ID: {key} -> {value}")
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades:
            print(f"The grade of student with ID {student_id} has been updated.\n")
        self.grades[student_id] = grade

    def remove_grade(self, student_id):
        if student_id in self.grades:
            del self.grades[student_id]
            print(f"Grade of student with ID {student_id} is removed.\n")
        else:
            print(f"Student with ID {student_id} does not have a recorded grade.\n")
