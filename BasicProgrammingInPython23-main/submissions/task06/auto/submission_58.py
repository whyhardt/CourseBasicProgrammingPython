...
def calculate_average(grades):
    # we are using the mean therefore the formula is the sum of all grades divided by the amount
...
# Considering we are supposed to prompt the user i have removed the example sets given

#by looking up the documentation we can find the underlying calculations of these functions
def union(set1, set2):
    return set1 | set2
...
# Prompt the user to enter the elements of set1 and set2
set1 = set(input("Enter the elements of set1 (space-separated): ").split())
set2 = set(input("Enter the elements of set2 (space-separated): ").split())
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades:
            print(f"Grade for student {student_id} updated.")
        self.grades[student_id] = grade

    def remove_grade(self, student_id):
        if student_id in self.grades:
            del self.grades[student_id]
            print(f"Grade for student {student_id} removed.")
        else:
