...
for i in range(num_cities):  
    city = input(f"Enter the name of city {i+1}: ") #ask for the names of the cities
    population = int(input(f"Enter the population of {city}: ")) #ask for the population of each city
    cities.append((city, population))  #add the city and population to the list 'cities'

# Calculate the total population
total_population = sum(population for _, population in cities) #the _ is a placeholer for the city names which we don't need, we only add the population of each city

# Find the city with the highest population
highest_city = max(cities, key=lambda x: x[1]) #finding the max by using a lambda function as a key to compare the population values

# Find the city with the lowest population
lowest_city = min(cities, key=lambda x: x[1]) #finding the min by using a lambda function as a key to compare the population values
...
def calculate_average(grades):
    total = sum(grades) 
    return total / len(grades) #average = sum divided by number of entries(=length of a list)
...
for student_grades in grades:
    name = student_grades[0] #students name is the first entry in each tuple (first entry=0)
    average_grade = calculate_average(student_grades[1:]) #calculate the average by excluding the name and only using the entries from 1 to the end
...
def count_unique_elements(lst):
    unique_elements= set(lst) #by using set() we create a list with no duplicate elements
    return len(unique_elements) #length of a list = number of elements
...
def add_book(collection, book_id, title, author, year):
    collection[book_id] = { #create a new entry in the list
        "title": title,
        "author": author,
        "year": year
    }
...
def remove_book(collection, book_id): 
    if book_id in collection:
        del collection[book_id]  #delete the entry in the list, if it is in there

def search_book(collection, book_id):
    if book_id in collection:  #print the searched book if it is in the collection
...
def display_all_books(collection):
    ...
    else:   #print all items in the list
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        ...
        self.grades[student_id] = grade #update grade

    def remove_grade(self, student_id):
        if student_id in self.grades:
            del self.grades[student_id]  #delete 
            print(f"Grade for student {student_id} has been removed.")
        else:
            print(f"No grade found for student {student_id}.")
