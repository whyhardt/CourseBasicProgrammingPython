...
for i in range(num_cities):
    ...
    city_pop = int(input(f"Enter the population of the {i+1}. city: ")) 
...
def calculate_average(student_grades):
    ...
    just_grades = student_grades[1::]
    return sum(just_grades)/len(just_grades)
...
def add_book(collection, book_id, title, author, year):
    collection.update({book_id: {"title" : title, "author" : author, "year" : year}})

def remove_book(collection, book_id):
    del collection[book_id]
def search_book(collection, book_id):
    try:
        print(f"Book ID: {book_id} Title: {collection[book_id]['title']} Author: {collection[book_id]['author']} Year: {collection[book_id]['year']}")
    except:
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        self.grade_tracker.update({student_id : grade})
    ...
    def display_grades(self):
        for elem in self.grade_tracker:
            print(f"{elem}: {self.grade_tracker[elem]}")
