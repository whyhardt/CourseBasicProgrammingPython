...
for i in range(num_cities):
    ...
    name_city = input(f"Enter the name of city #{i+1}: ")
    population = int(input(f"Enter the population of city #{i+1}: "))
    city = (name_city, population)
    cities.append(city)
    

# Calculate the total population
total_population = sum(population for name_city, population in cities)
...
def calculate_average(grades):
  student_grades = sum(grades)
  average_grade = student_grades/len(grades)
...
def symmetric_difference(set1, set2):
  return set1.symmetric_difference(set1)
...
def add_book(collection, book_id, title, author, year):
    details = {'title': title, 'author': author, 'year': year}
...
def display_all_books(collection):
    for book_id, details in collection.items():
        print(f"Book ID: {book_id}")
        print(f"Title: {details['title']}")
        print(f"Author: {details['author']}")
        print(f"Year: {details['year']}")
