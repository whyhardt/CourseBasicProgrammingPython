...
for i in range(num_cities):
    cities.append((str(input("Enter the City Name")), int(input("Enter the City Population"))))
    print(cities[i])

#cities = [("debug", 1000),("Osna", 6000)]

# Calculate the total population
total_population = sum(population for city, population in cities)

# Find the city with the highest population
highest_city = max(cities, key=lambda pop: pop[1])

# Find the city with the lowest population
lowest_city = min(cities, key=lambda pop: pop[1])
...
for student_grades in grades:
    name = grades[i][0]
    average_grade = calculate_average(grades[i][1:])
    i += 1
...
class StudentGradeTracker:
    ...
    def remove_grade(self, student_id):
        if student_id in self.grades:
            ...
            print("Error, student_id not found")
