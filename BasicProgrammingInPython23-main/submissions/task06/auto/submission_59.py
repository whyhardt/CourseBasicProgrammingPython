...
# **Your name here**: Jessica Diana Ryglewicz 
...
for i in range(num_cities):
    city = input(f"Enter name of {i+1} city and after ',' it's population")
    elements = tuple(city.split(","))
    elements = (str(elements[0]), int(elements[1]))
...
def search_book(collection, book_id):
    if book_id in collection:
        print(f"Your seearched book: {collection[book_id]}")
    else:
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        ...
        if student_id in self.grades:
            return "Grades has been updated"

    def remove_grade(self, student_id):
        if student_id in self.grades:
            self.grades.pop(student_id)
            return "Grade has been removed"
        else:
            return "Student does not have a grade"

    def display_grades(self):
        if not self.grades:
            return "No grades have been recorded"
        else:
            print("\n".join([f"{key}:{value}" for key, value in self.grades.items()]))
