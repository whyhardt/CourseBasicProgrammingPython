...
highest_city = max([city for city in cities])

# Find the city with the lowest population
lowest_city = min([city for city in cities])
    ...
    for student in grades:
        if student[0] == name:
            ...
            for grade in student:
                ...
                calculated_grade = calculated_grade + student[single_grade]
                if single_grade >= 4:
                    break
    calculated_grade = calculated_grade / 4
...
def add_book(collection, book_id, title, author, year):
    book_collection = {book_id: (title, author, year)}
...
def search_book(collection, book_id):
    ...
    print(collection)
    """title = details["title"]
    print(f"The title of the book is: " + details["title"] + 
          " it was written by: " + details["author"] + 
          " and published in the year: " + details["year"])"""

def display_all_books(collection):
    all_details = collection.values()
    print(all_details)
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grade_tracker:
            print("The grade of the student is updated.")
    ...
    def display_grades(self):
        print(self.grade_tracker.values())
