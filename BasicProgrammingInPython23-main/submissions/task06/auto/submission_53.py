...
total_population = sum([c[1] for c in cities])

# Find the city with the highest population
highest_city = max(cities, key=lambda x: city[1])

# Find the city with the lowest population
lowest_city = min(cities, key=lambda x: city[1])
...
def count_unique_elements(lst):
  ...
  for i in range(len(lst)):
    lst_copy = lst.copy()
    element = lst_copy.pop(i)
    if element not in lst_copy:
        count += 1
...
def search_book(collection, book_id):
    if book_id in collection:
        print(f'ID {book_id} was found.')
        print(f'Title: {collection[book_id][0]}')
    else:
        print(f'ID {book_id} was not found.')
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades.keys():
            print(f'the grade of student {student_id} got updated.')
        self.grades[student_id] = grade

    def remove_grade(self, student_id):
        if student_id in self.grades.keys():
            del self.grades[student_id]
            print(f'the grade of student {student_id} is removed.')

        else:
            print(f'student {student_id} does not have a record.')
            

    def display_grades(self):
        if not self.grades:
            print('there is no grade to display, no student has a record.')
        
        for student_id in self.grades.keys():
            print(f'-ID {student_id} : {self.grades[student_id]}')
