...
for i in range(num_cities):
    ...
    cities.append(city)
    population = int(input("Enter the population of city: "))
...
maximum = populations[0]  # Choose any point of the Population list
...
print("The maximum element in the population is: ", maximum)
idx_maximum = populations.index(maximum)
        
# Find the city with the lowest population
minimum = populations[0]  # Choose any point of the Population list
...
print("The minimum element in the population is: ", minimum)
idx_minimum = populations.index(minimum)
...
print(f"Name: {cities[idx_maximum]}")
print(f"Population: {maximum}\n")

# Print the city with the lowest population
print("City with the lowest population:")
print(f"Name: {cities[idx_minimum]}")
print(f"Population: {minimum}")
...
for student_grades in grades:
    num_elem = len(student_grades)
    name = student_grades[0]
    average_grade = calculate_average(student_grades[1:num_elem])
...
#Ask the user to input two sets( sets should have the same length)
num_elem = int(input("Enter Number of Elements in Sets:"))

set1 = []
set2 = []

for i in range(num_elem):
    in1 = int(input("Element for Set_1:"))
    set1.append(in1)
    in2 = int(input("Element for Set_2:"))
    set2.append(in2)

#Create Sets from user input
set1 = set(set1)
set2 = set(set2)
...
def count_unique_elements(lst):
    unique_value = set(lst) # in sets there can only be unique elements
    unique_elem = len(unique_value) # how many elements are in the set
    return unique_elem
...
def add_book(collection, book_id, title, author, year):
    collection[str(book_id)] = {'Title':title,'Author':author,'Year':year }
    
def remove_book(collection, book_id):
    del collection[str(book_id)] #Remove element with id element

def search_book(collection, book_id):
    if str(book_id) in collection:
        for k, v in collection[str(book_id)].items():
            print("%s : %s "%(k, v))

def display_all_books(collection):
    for keys in collection.keys():
        print("\nID: %s"%(keys))
        for k, v in collection[str(keys)].items():
            print("%s : %s "%(k, v))
...
class StudentGradeTracker:
    def __init__(self):
        self.student_dict = {} #Create a new student_grade dict instance

    def add_grade(self, student_id, grade):
        if student_id not in self.student_dict:
            self.student_dict[str(student_id)] = grade
        else:
            #The ID already exists
            self.student_dict[str(student_id)] = grade
            print("The Student with ID (%s) already exists, Grade was updated !"%(student_id))
            
    def remove_grade(self, student_id):
        if student_id not in self.student_dict:
            print("The Student with ID (%s) has no grade yet !"%(student_id))
        else:
            #The Student with ID exisists in the  dict
            del self.student_dict[str(student_id)]

    def display_grades(self):
        if len(self.student_dict)>0:
            #if there are actually elements in the dict
            for k, v in self.student_dict.items():
                print("%s : %s "%(k, v))
        else:
            #There are no elements in the dict
