...
for i in range(num_cities):
    city = input(f"Enter the name of a city {i + 1}: ")
...
def add_book(collection, book_id, title, author, year):
    ...
    collection[book_id] = book_details
...
def search_book(collection, book_id):
    searched_details = collection.get(book_id)
    if searched_details == None:
        print(f"The book with the id {book_id} does not exist anymore")
    else:
        print(f"These are the details for the book with the id {book_id}: {searched_details}")
...
class StudentGradeTracker:
    ...
    def remove_grade(self, student_id):
        if student_id in self.grade_tracker:
            ...
            print(f"Grade of student {student_id} is removed")
        else:
            print(f"The student {student_id} has no grade recorded")
