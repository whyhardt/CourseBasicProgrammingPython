...
for i in range(num_cities):
    ...
    lst = [city, pop]
    cities.append(lst)
poplist = [int(i) for i in poplist]


# Calculate the total population
total_population = sum(poplist)

# Find the city with the highest population
highest_pop = max(poplist)
ind = poplist.index(highest_pop)
highest_city = cities[ind]

# Find the city with the lowest population
lowest_pop = min(poplist)
ind = poplist.index(lowest_pop)
lowest_city = cities[ind]
...
def calculate_average(gr):
  ...
  averagegrade = (gr[1]+gr[2]+gr[3]+gr[4]) /4
  return averagegrade
...
def search_book(collection, book_id):
    ...
    if book_id in collection:
        ...
        print(f"Book with book id {book_id}: {book}")
        #print(f"Book ID: {book_id}")
        #print(f"Title: {book['Title']}")
        #print(f"Author: {book['Author']}")
        #print(f"Publication year: {book['Publication year']}")
    else:
        print(f"No book with book id {book_id} in the collection.")
    # Remove pass after the implementation
...
class StudentGradeTracker:
    ...
    def display_grades(self):
        ...
        if len(self.student_grade_tracker) > 0:
            print(self.student_grade_tracker)
        else:
