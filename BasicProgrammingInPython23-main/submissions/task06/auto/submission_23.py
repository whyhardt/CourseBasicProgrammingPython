...
# **Your name here**: Periklis Maskanakis   
# **Your university mail**: pmaskanakis@uos.de
# 
# ___
# 
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        exist = self.studend_grades.get(student_id)
        if exist is not None:
            print("grade has been updated")
        self.studend_grades[student_id] = grade
            

    def remove_grade(self, student_id):
        exist = self.studend_grades.get(student_id)
        if exist == None:
            print("student not found / student has no grade")
        else:
            del self.studend_grades[student_id]
            print(f"grade from student {student_id} has been removed")

    def display_grades(self):
        if len(self.studend_grades) == 0:
            print("Dictionary is empty")
        else:
            for i in self.studend_grades:
                print(f"ID {i} : {self.studend_grades[i]}")
