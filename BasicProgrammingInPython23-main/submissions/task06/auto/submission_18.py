...
for i in range(num_cities):
    ...
    city_population = int(input(f"Enter the population of {city_name}: "))
...
def add_book(collection, book_id, title, author, year):
    if book_id not in collection:
        collection[book_id] = {"title": title, "author": author, "year": year}
        print("Book added successfully.")
    else:
        print("Book with ID", book_id, "already exists.")
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades:
            print("Grade updated for student ID:", student_id)
    ...
    def remove_grade(self, student_id):
        if student_id in self.grades:
            ...
            print("Student ID", student_id, "does not have a grade recorded.")
