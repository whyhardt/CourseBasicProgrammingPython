...
for i in range(num_cities):
    name_city = input("Enter name of city: ")  
    population_city = int(input("Enter its population: "))
    cities.append([name_city, population_city])
...
for student_grades in grades:
    ...
    average_grade = calculate_average(tuple([grade for grade in student_grades[1:]]))
...
def search_book(collection, book_id):
    if book_id in collection:
        ...
        print(f"No book with id {book_id} in the collection")
        return
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades_dic:
            print("Grade has been updated")
        self.grades_dic[student_id] = grade

    def remove_grade(self, student_id):
        if student_id in self.grades_dic:
            print("Grade has been removed")
            del self.grades_dic[student_id]
        else:
            print("There is no grade for this student yet")
            
    def display_grades(self):
        if not self.grades_dic:
            print("No grades have been recorded yet")
        else:
            for key in self.grades_dic:
                print(f"{key}: {self.grades_dic[key]}")
