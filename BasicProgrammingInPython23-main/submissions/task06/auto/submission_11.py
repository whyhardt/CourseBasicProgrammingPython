...
for i in range(num_cities):
    name = input(f"Enter the name of city number {i+1}: ")
    population = int(input(f"Enter the poulation of {name}: "))
    cities.append((name, population))
    
# Calculate the total population
total_population = sum(population for name, population in cities) # iterating over cites and summation of population

# Find the city with the highest population
highest_city = max(cities, key=lambda x: x[1]) #lambda checks for the next index to compare values
...
def calculate_average(grades):
    ...
    average = sum(grades[1:]) / len(grades[1:]) # sum of all grades through total number of grades
    return average
...
for student_grades in grades:
    ...
    average_grade = calculate_average(student_grades[1:]) #except name
...
def difference(set1, set2):
    ...
    set_difference = set_diff1.union(set_diff2)
    return set_difference
    
def symmetric_difference(set1, set2):
    set_symmetricdiff = set1.symmetric_difference(set2)
    return set_symmetricdiff
...
numbers = [int(num) for num in user_input if num.strip()]
...
def add_book(collection, book_id, title, author, year):
    ...
    print("Book was added to collection")
    
def remove_book(collection, book_id):
    if book_id in collection:
        del collection[book_id]
        print("Book was removed from collection")
    else:
        print("Book does not exist in collection")
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        ...
        self.grade = grade
        self.student_tracker[self.student_id] = self.grade
        if self.student_id in self.student_tracker:
    ...
    def remove_grade(self, student_id):
        self.student_id = student_id
        if self.student_id in self.student_tracker:
            del self.student_tracker[self.student_id]
            print("Grade has been removed")
        else:
    ...
    def display_grades(self):
        if self.student_tracker:
            ...
            for student_id, grade in self.student_tracker.items():
