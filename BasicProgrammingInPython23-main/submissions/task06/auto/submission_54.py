...
# **Your name here**: Felix Böhm
# **Your university mail**: felboehm@uos.de
# 
# ___
# 
...
def city_tracker(city_list):
    ...
    for i in range(num_cities):
        ...
        populations.append(city_list[i][1])
    # Calculate the total population
    total_population = sum(populations)

    # Find the city with the highest population
    highest_city = max(map(lambda x: x[1], cities) )
    print(highest_city)
    # Find the city with the lowest population
    lowest_city = min(map(lambda x: x[1], cities) )
    print(lowest_city)
...
city_list = [("Sydney", 2000000), ("Berlin", 1000000), ("Hamburg", 2000000), ("Osnabrück", 90000)]

city_tracker(city_list)
...
set1 = list(input("Enter the set1 values: "))
set2 = list(input("Enter the set2 values: "))
set1 = set(map(lambda x: int(x),set1))
set2 = set(map(lambda x: int(x),set2))
...
def search_book(collection, book_id):
    print("Book Name: ", collection[book_id]['title'])
    print("Book Author: ", collection[book_id]['author'])
    print("Book Published: ", collection[book_id]['year'])

def display_all_books(collection):
    for x in collection:
        print("Book Name: ", collection[x]['title'])
        print("Book Author: ", collection[x]['author'])
        print("Book Published: ", collection[x]['year'])
...
add_book(book_collection, 1, "Python Programming", "LX & BoneZ", 2022)
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.student_grades.keys():
    ...
    def remove_grade(self, student_id):
        if student_id in self.student_grades.keys():
            ...
            print("The given StudentID does not have grades on record")

    def display_grades(self):
        if not self.student_grades:
            print("There are no recorded grades")
        else:
            for x in self.student_grades:
                print("StudentID :", x)
                print("Grade :", self.student_grades[x])
