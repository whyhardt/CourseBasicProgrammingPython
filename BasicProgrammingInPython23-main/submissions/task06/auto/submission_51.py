...
for i in range(num_cities):
    ...
    pop = input("Enter the population of the last named city: ")
    each = (name, pop)
    cities.append(each)
    population.append(pop)

# Calculate the total population
total_population = sum(int(i) for i in population)
...
def calculate_average(in_grades):
  total = 0
  for i in range(5):
     if i == 0:
        continue
...
def count_unique_elements(lst):
  ...
  # add elements to 
  # unique list when not already included
  # helper list when already in unique list
  for elem in lst:
  ...
  # remove elements from unique again when also in helper
  for elem in unique:
    if elem in helper:
      unique.remove(elem)
...
def search_book(collection, book_id):
    if book_id in collection:
        details = collection.get(book_id)
        return print(details)
    else: 
        print("No book with the given ID was found")
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.dict_grade:
            print("The grade has been updated.")
        self.dict_grade[student_id] = grade
        return 

    def remove_grade(self, student_id):
        if student_id in self.dict_grade:
            del self.dict_grade[student_id]
