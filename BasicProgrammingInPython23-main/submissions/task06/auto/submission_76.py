...
for i in range(num_cities):
    #your code here  
    cities.append((input("Enter a city: "), int(input("Enter the corresponding population: "))))
...
def calculate_average(stud_grades):
...
def search_book(collection, book_id):
    ...
    # if the try part doesn't work, it throws a KeyError -> inform the user that the ID does not exist in collection
    except KeyError:
...
class StudentGradeTracker:
    def __init__(self):
        # Add implementation here
        self.student_grade_tracker = {}

    def add_grade(self, student_id, grade):
        # Add implementation here
        if student_id in self.student_grade_tracker:
            print(f"The grade for the student ID {student_id} has been updated!")
        self.student_grade_tracker.update({student_id : grade})

    def remove_grade(self, student_id):
         # Add implementation here
        if student_id in self.student_grade_tracker:
            print(f"The grade for the student ID {student_id} has been removed!")
            del self.student_grade_tracker[student_id]
        else:
            print(f"The student with the student ID {student_id} does not have a grade recorded!")
        

    def display_grades(self):
        # Add implementation here
        if len(self.student_grade_tracker) > 0:
            for student_id in self.student_grade_tracker:
                print(student_id, ": ", self.student_grade_tracker[student_id])
        else:
