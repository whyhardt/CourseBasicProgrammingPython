...
# **Your name here**: Dominik Brockmann  
# **Your university mail**: dobrockmann@uni-osnabrueck.de
# 
# ___
# 
...
for i in range(num_cities):
    name = input(f'Name of city number {i+1}: ')
    population = int(input(f'Population of city number {i+1}: '))
    cities.append((name, population))

# Calculate the total population
total_population = sum([p for _, p in cities])

# Find the city with the highest population
highest_city = max(cities, key = lambda k: k[1])

# Find the city with the lowest population
lowest_city = min(cities, key = lambda k: k[1])
...
for s in [set1, set2]:
  while True:
    inp = input('Enter a value or enter nothing to finish this set: ')
    if inp == '':
      break
    s.add(int(inp))
...
# and convert the numbers from strings to integers
...
def remove_book(collection, book_id):
    collection.pop(book_id)

def search_book(collection, book_id):
    if book_id in collection:
        return collection[book_id]
    
def display_book(collection, book_id):
    print(book_id, ': ', sep='', end='')
    print(collection[book_id]['title'], 'by', collection[book_id]['author'], f'({collection[book_id]["year"]})')

def display_all_books(collection):
    for book_id in collection:
        display_book(collection, book_id)
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grades:
            print('Student exists already, the grade will be updated accordingly.')
        self.grades[student_id] = grade

    def remove_grade(self, student_id):
        x = self.grades.pop(student_id, None)
        print('Grade successfully removed.' if x != None else 'Student has no recorded grade.')
