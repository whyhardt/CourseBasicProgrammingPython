...
for i in range(num_cities):
    ...
    population = int(input("Enter it's population"))
...
user_input = str(input("Enter a list of numbers (separated by spaces): "))
...
def add_book(collection, book_id, title, author, year):
    collection = collection.update(book_id = key; title, author,year = value)
    return

def remove_book(collection, book_id):
    collection = collection.pop(book_id)
    return
...
def display_all_books(collection):
    collection = {tuple(book_collection)}
