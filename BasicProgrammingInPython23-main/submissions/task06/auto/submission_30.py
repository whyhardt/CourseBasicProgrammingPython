...
for i in range(num_cities):
    ...
    pop_number = int(input("Population:"))
    cities.append([city_name, pop_number])

# Calculate the total population
total_population = sum([pop for _, pop in cities])
...
def add_book(collection, book_id, title, author, year):
    # Add implementation here
    # Remove pass after the implementation
    collection[book_id] = {'title': title,
                           'author': author,
                           'year': year}
...
def search_book(collection, book_id):
    ...
    if book_id in collection.keys():
        ...
        print("No book with the ID", book_id)

def display_all_books(collection):
    # Add implementation here
    # Remove pass after the implementation
    for id, book in collection.items():
        print(f'Book ID: {id}', *[f'- {key}: {value}' for key, value in book.items()])
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        # Add implementation here
        # Remove pass after the implementation
        if student_id not in self.grades.keys():
            self.grades[student_id] = grade
        else:
            print("Grade has already been updated!")
    ...
    def remove_grade(self, student_id):
        ...
        if student_id in self.grades.keys():
            ...
            print("Student does not have a grade recorded.")

    def display_grades(self):
        # Add implementation here
        # Remove pass after the implementation
        if len(self.grades.keys()):
            for id, grade in self.grades.items():
                print(f"{id}: {grade}")
        else:
            print("No grades are recorded.")
