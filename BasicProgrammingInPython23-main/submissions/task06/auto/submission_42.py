...
#total_population = sum(population[1] for population in cities)
...
def calculate_average(grades):
    return sum(grades)/len(grades)   # Add implementation here
...
for student_grades in grades:
    ...
    average_grade = calculate_average(student_grades[1:]) # Add implementation here
...
def symmetric_difference(set1, set2):
    ...
    dif2 = set2.difference(set1) # Add implementation here
...
def add_book(collection, book_id, title, author, year):
    return collection.update({book_id: {"title": title, "author": author, "year": year}})


def remove_book(collection, book_id):
    return book_collection.pop(book_id)

def search_book(collection, book_id):
    if book_id in collection:
        print(book_id, ": ", book_collection[book_id]) 
    else:
        print("Sorry, the book is not in this collection")
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.grade_tracker:
            ...
            print("The grade for ", student_id, "has been updated")
        else:
            self.grade_tracker[student_id] =  grade

    def remove_grade(self, student_id):
        if student_id in self.grade_tracker:
            self.grade_tracker.pop(student_id)
            print("The grade for ", student_id, "has been deleted")
        else:
            print("No grade on record")

    def display_grades(self):
        if self.grade_tracker == {}:
            print("there are no grades on record")
        else:
            for student_id in self.grade_tracker:
                print(student_id, ":", self.grade_tracker[student_id])
