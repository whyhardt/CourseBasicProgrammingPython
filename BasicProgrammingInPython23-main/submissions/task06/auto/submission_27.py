...
# **Your name here**: Lukas Gruber  
# **Your university mail**: lugruber@uos.de
# 
# ___
# 
...
# I do not see why it makes sense to use an if-condition to determine the cities with the highest and lowest population.
...
for i in range(num_cities):
    ...
    pop = int(input("Please enter the number of inhabitants: "))
    tuple = (city, pop)
    cities.append(tuple)  

# Calculate the total population
total_population = sum(tuple[1] for tuple in cities) 

# Find the city with the highest population
highest_city = max(cities, key = lambda tuple: tuple[1])

# Find the city with the lowest population
lowest_city = min(cities, key = lambda tuple: tuple[1])
...
def intersection(set1, set2):
  return set1.intersection(set2)  # Remove pass after the implementation
...
class StudentGradeTracker:
    ...
    def remove_grade(self, student_id):
        if not student_id in self.grade_dict:
            print("The student does not have a grade recorded")
        elif self.grade_dict[student_id] == "removed":
            print("The grade has already been removed")
        else:
            self.grade_dict[student_id] = "removed"
    ...
    def display_grades(self):
        if self.grade_dict == {}:
            ...
            for i in self.grade_dict:
                print(i, ": ", self.grade_dict[i])
