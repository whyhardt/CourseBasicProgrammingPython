...
cities = [('Berlin',3500000),('Istanbul',15000000),('Osnabrück',1600000),('Trento',120000),('Rom',2800000),('Neapel',3800000)]
#6
...
def calculate_average(noten):
    anoten = ("Alice", 85, 90, 92, 88)
    noten = noten[1:len(noten)]
...
def display_all_books(collection):
    for ids,books in collection.items():
        print(ids,books)
...
#search_book(book_collection, 2) #why would you look up sth that was just deleted
...
class StudentGradeTracker:
    ...
    def add_grade(self, student_id, grade):
        if student_id in self.tracker.keys():
            print('The grade has been successfully updated!')
        self.tracker.update({student_id:grade})

    def remove_grade(self, student_id):
        if student_id in self.tracker.keys():
            del self.tracker[student_id]
            print('The grade has been successfully removed!')
        else:
            print('This student doesnt seem to have a record with us')
    ...
    def display_grades(self):
        ...
        if records_available:
            for e in self.tracker.keys():
                grade = self.tracker[e]
                #print('f{}:{}',(e,grade))
                print(f"{e}:{grade}")
        else:
            print('We dont currently have any stundent records available')
