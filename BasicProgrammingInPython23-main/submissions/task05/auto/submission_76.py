...
class BankAccount:
    ...
    def __str__(self):
        ...
        return f"""
               The account {self.account_number} has a balance of {self.balance}.
               """
    
    def deposit(self, amount):
        '''Method for depositing into the account.'''
        amount=input('Type the amount for deposit: ')
        self.balance=self.balance+int(amount)
        return self.balance
    
    def withdraw(self, amount):
        '''Method for withdrawing from the account.'''
        amount=input('Type the amount for withdraw: ')
...
account = BankAccount("123456789", 150)
...
counter = 0
start = datetime(year=2023, month=5, day=22, hour=13, minute=45, second=counter)
