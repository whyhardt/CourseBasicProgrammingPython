...
student = Student('student1',32,18)
...
class Triangle:
    ...
    def is_equilateral(self):
        ...
        all_edges = [self.a,self.b,self.c]
        all_edges.remove(self.width)
        if all_edges[0] == all_edges[1]:
...
while time_passed < 60 :
    ...
    diff = now_minute - start_minute
    if diff >= 0 :
        time_passed = (diff)*60 + (now_second - start_second)
    else:
        diff = 60 - diff
        time_passed = (diff)*60 + (now_second - start_second)
    
    print (time_passed,end="\r")
    counter += 1
