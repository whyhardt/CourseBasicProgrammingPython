...
for char in my_string:
    print(char)
    # Check if the character is a vowel and increase the counter if it is
...
# Would be shorter with alphabet.append("H"), but due to the wording I assume we are supposed to use insert()
# You could also insert the variable removed from above instead of "H" if you run the cell above before this one.
...
names = []
user_input = input("Enter a name or \'stop\', to stop: ")

# Check if the input is "stop" in each iteration
while user_input != "stop":
    # Add the input to the list and ask for the next input
