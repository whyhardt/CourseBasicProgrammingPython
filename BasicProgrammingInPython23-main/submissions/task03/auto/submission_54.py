...
result = []
number = 0
while number < 21:          #while loop as long as the number is smaller than 21
    if number % 2 == 0:     #if the number is even..
        result += [number]  #... the number gets added to our result list
    number += 1             #the number gets increased by 1 to check all other numbers till 20
...
for word in python_text:
    if len(word) > 8:       #if the length of the word is bigger than 8...
        result.append(word) #... the word is added to the result list
...
while True: 
    name = input("Enter a name or 'stop' to stop: ") #asking the user for input of names of "stop" to stop
    if name == 'stop':
        break          # break statement to end the loop if "stop" was entered
    names.append(name) #appends inserted names to the list names
