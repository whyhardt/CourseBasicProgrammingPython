...
names.sort() #sort function is by alphabetical order
...
alphabet.insert(len(alphabet)+1, "H")
#alphabet.append('H')
...
while stopp:
    names += [input("Insert a name here or \'stop\' to stop: ")]
    if names[i] == "stop" or names[i] == "Stop":
        names.pop(i) #deleting input stop from the list so it is not taken into account
        stopp = False
    i += 1
