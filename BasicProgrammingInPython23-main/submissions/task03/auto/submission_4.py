...
for i in range(0,len(my_string)):
    print(i, my_string[i])
    if ('A' in my_string[i] or 'E' in my_string[i] or 'I' in my_string[i] or 'O' in my_string[i] or 'U' in my_string[i] or 'a' in my_string[i] or 'e' in my_string[i] or 'i' in my_string[i] or 'o' in my_string[i] or 'u' in my_string[i]):
        vowel_count += 1
...
for i in range (0, len(python_text)):
    if len(python_text[i]) > 8:
        result += [python_text[i]]
...
while new_name !='stop':
    new_name = input("Please enter the name of the person :- ")
    names.append(new_name)
    
    if new_name == 'stop':
        
        if names == ['stop']:
            # If the 'names' list only has the word 'stop' or 'Stop', then the user will be requested to input atleast one name.
           print("Please enter the name of atleast one person. ")
        
        else :
            # If the word 'stop' has been entered after entering some names, then names.pop() will remove the 'stop' word from 
            # the 'names' list, so that the random module discards the word 'stop' and only takes the person names.
            names.pop()
